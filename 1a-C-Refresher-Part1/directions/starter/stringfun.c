...
// So that you can use them later on?, Or is it a memory allocation thing?
// According to geeksfoegeeks, it helps the complier to cross-checks function parameters and their data type
...
int count_words(char *str){
    ...
    int len; // Did not use this at all
    int wc;
...
void  word_print(char *str){
    ...
    for (int i = 0; i < len; i++) {
        char current_character = str[i];
      //printf("Current letter: %c ", current_character);
      // I learned that order matters.
        if (word_start && (current_character == SPACE_CHAR)) {
            word_start = false;
        }
...
int main(int argc, char *argv[]){
    ...
    switch (opt){
        ...
        case 'r':
            ...
            // String is reversed in place due to the swapping of the characters.
            // A pointer to a location where a string is located is modified which means we don't need to return anything?
            break;
        case 'w':
            printf("Word Print\n----------\n");
        ...
        //          your thoughts.
        // It lets the person know that they entered an invalid option/
        default:
            usage(argv[0]);
    ...
    // I think it would run through all the options
}