...
// https://www.geeksforgeeks.org/memcmp-in-c/
...
int get_student(int fd, int id, student_t *s)
{
    ...
    // Looking for spot in database for target student
    if (lseek(fd, offset, SEEK_SET) == (off_t)-1) {
        return ERR_DB_FILE;
    } 

    // Read the next line of student_t into s
    int bytesRead;

    while ((bytesRead = read(fd, s, sizeof(student_t)) > 0)) {
        if (s->id == id) {
            return NO_ERROR;
        }
...
{
    // TODO
    student_t new_student, temp_student;
    int offset = id * sizeof(student_t);
    ...
    // Read the next line of student_t into s
    int bytesRead = read(fd, &temp_student, sizeof(student_t));
    ...
    //printf("Bytes read: %d\n", bytesRead);
    //printf("Read Student ID: %d, Name: %s %s, GPA: %d\n", temp_student.id, temp_student.fname, temp_student.lname, temp_student.gpa);
    

    if (bytesRead == 0 || temp_student.id < 0 || temp_student.gpa < 0 || temp_student.fname[0] == '\0' || temp_student.lname[0] == '\0') {
        ...
        int bytesWrite = write(fd, &new_student, sizeof(student_t));
    ...
    //lseek(fd, offset, SEEK_SET);
    //bytesRead = read(fd, &temp_student, sizeof(student_t));
    //printf("Bytes read: %d\n", bytesRead);
    //printf("New Read Student ID: %d\n", temp_student.id);
    //printf("New Expected Student ID: %d\n", new_student.id); 
...
int del_student(int fd, int id)
{
    ...
    // Read the next line of student_t into s
    int bytesRead = read(fd, &student, sizeof(student_t));
    // Error Reading File
    if (bytesRead == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    ...
    lseek(fd, offset, SEEK_SET);
    int bytesWrite = write(fd, &EMPTY_STUDENT_RECORD, sizeof(student_t));
...
int print_db(int fd)
{
    ...
    // There should be a more effective way to do this right? Like a function or something?
    while ((bytesRead = read(fd, &student, sizeof(student_t)) > 0)) {
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            if (count == 0) {
...
int compress_db(int fd)
{
    ...
    while ((bytesRead = read(fd, buffer, buffer_size)) > 0) {
        if (write(new_fd, buffer, bytesRead) != 0) {
            printf(M_ERR_DB_WRITE);
            return ERR_DB_FILE;
        }
    }
